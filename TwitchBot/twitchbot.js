		...
		setTimeout(function () {
			...
			for (var i = 0; i < songs.length; i++) {
				if (songs[i].indexOf('">') != -1) {
					...
					// console.log(songs[i]);
					fs.appendFile('./html/list.html', "\r\n <tr><th>" + i + " </th><td> " + songs[i] + "</td></tr>", function (err) {});
				}
			}		
					...
					if (songWord.length == 0 || message.substring(RequestTxt.length + 1) == false || message.substring(RequestTxt.length + 1) == "***" || message.indexOf("!노래신청 ") != 0) {
						...
						if (songPossible.length > 0) {
							if (songIsInCoolDown[songIndex] === true && username != Managername) {
								...
								if (vipList.indexOf(username) != -1) {
									sendMessage(client, channel, 'VIP 노래 신청은 ' + UserCooldown / 60 * 0.5 + '분 마다 가능합니다... @' + username);
								} else {
...
function makeFooter(){
	fs.appendFile('./html/list.html', "</table></tbody></html>", function (err) {});
}

makeHeader();
mainFunc().then(makeFooter); 
